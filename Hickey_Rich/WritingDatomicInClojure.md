...
[The keynote he mentions above is probably Rich Hickey's keynote talk
"The Value of Values".  He gave a 1 hour version of a talk with that
name at the GOTO Copenhagen Conference 2012, and a 30 minute version
later at Jax Conference 2012 in San Francisco.]
...
So why do this?  For at least these two reasons.  We want to pursue
...
[ TBD - what is time point for this image switch? ]
![0:04:09 Architecture Sharded Client-Server](WritingDatomicInClojure/DB-Architecture-4-Sharded-Client-Server.jpg)

So you start to lose.  You start sharding, and you cannot query
...
![0:04:54 Architecture Sharded Client-Server](WritingDatomicInClojure/DB-Architecture-5-KV-Store.jpg)

And then we move to the newer generation of things that say: by the
time you are sharding you are not getting many of those advantages.
Let us just have key/value stores.  TBD independence.  We are just
going to consistent hash your keys and then find the machine that they
are going to run on.  And now we have independence here.

[TBD - what time point for this image?]
![0:04:54 Architecture Sharded Client-Server](WritingDatomicInClojure/DB-Architecture-6-KV-Store.jpg)
...
There is no leverage in a key-value store.  It is just gone.  This is
...
![0:06:19 Datomic Architecture](WritingDatomicInClojure/Datomic-Architecture-2.jpg)
...
are going to do with queries is place them in the application servers.
So now every application server gets its own brain.

[TBD - somewhere in here the figure changed the bidirectional lines
between App boxes and distributed storage service servers, to one
direction from servers to Apps.]

![0:06:xx Datomic Architecture](WritingDatomicInClojure/Datomic-Architecture-3.jpg)

But we still do not have transactions or consistency at this point.
But you will see if you move from the key-value store, every
participant of a key-value store could write to the key-value store
and read from it.  We are going to change that.  We are going to say:
you can only read from the storage service.
...
[Add a box labeled "Transactor"]
```

![0:06:55 Datomic Architecture](WritingDatomicInClojure/Datomic-Architecture-4.jpg)
...
This is a hybrid model that has sort of a traditional model for
...
![0:07:33 Datomic Architecture](WritingDatomicInClojure/Datomic-Architecture-5.jpg)
...
[TBD another small figure addition]
```

![0:07:39 Datomic Architecture](WritingDatomicInClojure/Datomic-Architecture-6.jpg)

So when an application has some novelty, write new facts, it is going
to send it to the transactor.  The transactor is going to broadcast
this back to other peers, we call them, that are connected.  And also
...
One way to look at this is sort of take the traditional architecture,
and see where they end up in this model.

So if we look at a traditional database, it has this monolithic model.
It does indexing.  It does transactions.  It does I/O to storage.  It
handles query load.  Your application itself, it does not have much
power at all.  It is completely relying on the server to do all of the
thinking.

But it often has a caching layer, or something like that, because this
unit [the server] gets overloaded.  So applications would go issues
queries, and then say: wow, that was expensive.  Maybe somebody else
will ask this same question.  I will manually go and put that answer
in a cache.  And then I will manually check the cache before I do
this, and it is all sort of "on me" to add this layer to avoid
overloading this central resource.
...
is ephemeral.  You can die.  Failure is a continuous state.  So you
want to design for that.  You want to design for unreliable disks.
...
to solve every problem related with databases.  You are just going to
get a million implementations of how to put blocks on the disk, and
...
support Infinispan.  And there are a bunch of other key-value stores
like CouchBase or Riak that make logical sense in this role as a
storage service.
...
+ Fewer peers, less power, less cost
+ Demand-driven
  + No configuration
```
...
database.  Let us take a bunch of monolithic databases.  And that is
...
So in the query case, because we have put query capability in each
...
Contrast that with saying: I am going to pre-configure a cluster, or I
am manually going to go to a console and add a machine to a cluster,
so I can have a bigger clustered server.  That is a very explicit
...
query, a communication engine, and a caching engine all built in to
your application servers by including the peer library.
...
people really appreciate how much client-server has invaded how much
they think about what applications can do, and what is OK.  Because
there is this conversational aspect, and what you do is expensive, and
you think that you are tapping into a shared resource that is
potentially getting overloaded, really compromises what you think your
application is allowed to do.

And when you move this stuff local, it is completely different.  If
you have your own app server, and it is doing analytics, and it needs
to run queries that run a long time, is that OK?  Now all of a sudden
it is completely OK.  Those queries do not tie up any shared
...
So ad hoc queries, long running queries, it all becomes OK because you
...
From a logic side, again, one of the missions statements was to bring
the power of programming with data into the application.  That is what
we are trying to do.  So we have a declarative logic built into the
application peers.
...
But the point now is that this is part of your application.  So this
is a really great thing.  SQL is a really great thing.  It is set
oriented.  It is declarative.  It is very powerful.  But it has always
been "over there", in a different server, in a different language.
You have to send strings to it.  You get something back.  It is
conversational.  There is a lot about that that does not make it
useful to you in your application.  It would be nice if you could use
it in your application, and that is what we have done.
...
applications are clearer, and easier to make correct.
...
inside a peer.  You can get your own transactions as well as
...
sharding.  There is no separation there.  There are no per-document
transactions, or any per-document set, or anything like that.
...
this is part of what it interesting about this from a Clojure
perspective, is we are actually trying to present the entire database
to the application as a value.  As a first class value, like I talked
about: completely immutable.  You do not see any other changes.  Every
part about it is evident.  It is comparable, and all of those other
kind of characteristics.

And so the way we achieve that is to make sure all of the data that is
kept in the storage service is immutable.
...
particular sore spot for databases, that they are not really
...
in and data out.  They are defined in terms of data, not in terms of
operations.
...
granularity of a fact is a row, or a document.  They are too big.  If
you are going to say: I want to record change, unless you have an
independent delta scheme, you have got to say: I have got a new
address.  Let me save "you" again.  That does not make sense.  You
need something that can represent just a "new email address".
...
And rather than store a time of day component here, what we do is
...
[Audience member: Is that a setting thing that you define for you as
you use it, or is it a run-time "I want a new attribute of this?"]

You can make a new attribute, and attributes are data.  Attributes are
facts, right?  They are stored in the database just like, and along
side, everything else, completely queryable and time coordinated to
everything.  They are just data.  They are just facts.

[Audience member: Every element had a time.]

Every element has a transaction that it occurred in, and transactions
have time associated with them.

[Audience member: TBD time TBD]
...
change them.  There is just a new fact.  I moved, so that does mean:
my address is no longer this.  But just saying "I have a new address"
is sufficient to imply that it is not the old address, and cardinality
can help make that automatic, too.  If you have a cardinality one
thing, and you now have a new value of it, it means the last value has
been superseded.
...
[TBD - get link to "Stu's talk"]

That talk is not the next talk.

[ "next talk" refers to "Get Logical with Datalog", Stuart Halloway
...
![0:24:32 Architecture](WritingDatomicInClojure/Architecture.jpg)
...
long as you are able to talk about it as of a particular point in
time.
...
incredibly inefficient.  You cannot keep rewriting the head of
something.
...
It keeps a sorted set in memory.  It keeps a sorted set on disk.  And
then every now and then it will merge from memory to disk.  When you
ask it a question, it always answers the question by giving you a live
merge of memory plus disk.
...
just use way too much storage, and you are constantly garbage
...
been made durable, but it has not necessarily been merged into durable
index.  And until it has been, it is kept in memory.
...
big internal nodes, so it works more like some of the other Clojure
data structures.

And it has pluggable comparators that you would expect.  And there are
always two of these maintained.  So without any user intervention, we
are always maintaining all of the datoms in entity, attribute, value,
time sort [EAVT], and attribute, entity, value, time sort [AEVT].  And
then there are ways to get, or request, attribute value sorts [AVET],
and the reverse indexing.  We do reverse indexing automatically for
any entity to entity references.  So for "Sally likes Fred", there is
a fast way to get from Fred to the fact that Sally likes Fred.
...
So we can look at a little bit about what we do in storage itself.
Unsurprisingly, we use storage like a tree.  Unlike BigTable, which
takes the segments that it produces in memory, and does another big
merge sort into another big block on the disk, we are not that disk
centric.  We are anticipating using more key-value store like storage
engines.  So we want to keep the block orientation that is traditional
in databases.
...
equivalent of atoms and refs.  And that puts the requirement on the
storage system that they offer consistent read, so we can implement
atoms, and conditional put: CAS, or something like it, so we can
...
as data comes in, it is immediately put in the log.  This is the thing
that is periodically updated.  And again, it is not updated in place.
Everything we put in here is immutable.
...
tree, and it is very shallow.  So there are only three levels, no
matter what.  There is a root, which is a pointer to a bunch of
directories, which are a bunch of pointers to segments.  Each segment
is a bunch of sorted datoms.  So sorted, compressed datoms.

And it is these things that we store into key-value like storage.  So
a whole big block, like 64K, of datoms go into storage at a time, into
storage.
...
said the application has a database value.  So we know that a map is
in Clojure, because it is all in memory.  So how do we do a value like
that, to present to the application, that is actually backed
essentially, partially, by storage, and has I/O associated with
getting that storage.

And that looks like this.  There is a root that is the DB value
object, which is stored in an atom.  This is it.  This is all the
...
It has got a pointer to that memory index.  So this memory index is
that persistent sorted set -- a set of persistent sorted sets.  And it
has got a pointer to a hierarchical cache that is backed, on cache
miss, with I/O to the storage system.  And that is again in parallel.
So there is going to be EAVT up here, AEVT down here, AEVT, AEVT and
everything else.  So there is the new, the novelty is accumulating in
memory.  And everything that accumulated before is happening here.
...
So that is sort of the storage side.  The other side of the system is
...
is race condition, potentially, with somebody else who is trying to
adjust your balance.
...
![0:33:57 Process Expansion](WritingDatomicInClojure/Process-Expansion.jpg)
...
"adjust account balance for Fred", then retract, assert, assert.  And
then this is turned into a two step process that was do this, and then
do that.
...
There is ground for data, which is assertions and retractions.  And
...
So a transactor accepts transactions.  It serializes transactions.
...
[ TBD - What research is this?  Try to find reference and cite it
here. ]
...
A the point it has got an acceptable transaction, it needs to log it.
At that point, the transaction happened.  So it has been made durable.
And it will then broadcast the change to everybody who has transacted,
...
in the middle of another file.  What happens when we make a new index
is: some nodes of the other tree are now no longer referenced from the
new tree.  So we just say: we do not care about them, and delete them
wholesale.  There is no merging.  There is no recreating a big master
thing again.  And some of the big long rewrites associated with things
like Cassandra, for instance, they go away, because we are not taking
the BigTable approach to merging.
...
pipelined.  So I talked about programming with data, and the ability
to use queues to pipeline you architecture internally.  That is what
happens inside the transactor implementation.  There is a lot of
pipelining.

What is interesting is: we say, ooh, couldn't you make the transactor
...
cores?  You pipeline your architecture.  You say there will be a
thread that is just expanding transactions, a thread that is applying
...
thread that is doing the acknowledgment.  Now you are using all of
the cores in your box efficiently, but you are still doing what is
effectively a serial process with just a lot of pipelining inside it.

And a lot of stuff is done asynchronously.

And then of course the other nice thing about being Clojure is all of
the storage engines we want to use have Java APIs, so just use them.
...
tree that is in storage that need to be replaced because we have data
to merge into them.  And that process is done in parallel.  It uses
"pmap" because it deals with a data set that is bigger than what fits
in memory, but we want to have parallelism.
...
do not want a serialized approach to dealing with the storage engine.
So again, a ton of parallelism in the indexing job, but it all uses
ordinary Clojure stuff.
...
more.  But while the indexing job was going on, more transactions came
...
need to say: of that novelty, I only care about the stuff that
happened since I started the indexing job.  Rather than have a
contention issue coming back from indexing, we just put the indexing
results into the input queue, because it is being serially processed.
So then you are going to process transactions, see the result of an
indexing job, do what we call the acceptance of that, which will nuke
the fact that you do not care about this memory index, and not have
any contention.  So we can still do it with one atom.
...
semantics.  They are meant to be used sparingly, and this shows how
tiny little bit of it you need for a very large system.
...
On the peer side, one of the things we said is that we have
declarative programming, so we have embedded Datalog.  It is a new
language, essentially, that I had to write, which is beautiful and fun
to write in Clojure.  It takes data sources and rule sets as
arguments.  So if you are used to Cascalog, it sort of has an ambient
notion.  It uses actually being embedded in Clojure, and name
resolution in Clojure, to sort of figure out what the data sources
are.  This is callable from Java so everything is explicit.  If you
have a structure you want to operate on, you pass it.  If you have a
rule set you want to use, you are going to pass it.
...
queries and so forth, because the interface is actually defined in
terms of data structures.  So that should be familiar.

The implementation -- if you know anything about Datalog, there are
sort of two fundamental ways to go about doing Datalog.  One is query
/ subquery recursive, and the other one is -- blanking on me now --
magic sets.  Query / subquery is actually a great fit for Clojure, in
...
is result at a time, or tuple at a time.  The semantics of Datalog are
set at a time.  That means that underneath the hood inside queries
that run here, entire sets are being merge joined.  And the things
that you expect of a query engine, like a hash join, a true hash join.
So I could have M and N, instead of doing M times N lookups, I am
going to take N, put it into a hash table, whatever the join
characteristic is, and then I am going to have M plus N complexity.
That kind of thing, which you never write by hand yourself in your
applications, you want from a database engine.  And it is possible in
Datalog, but not in the semantics of Prolog.

So it does that, and therefore is really fast.
...
So I talked about the difference between over here with over there,
...
They have this query engine.  They have this database that I
described: the combination of a live index and a storage backed index
being live merged.  That happens in the transactor, for transaction
support.  It happens in the peers for query support.  The exact same
thing.  The exact same components are present in the peer to represent
the database.
...
database.  It does not keep up with the whole database.

But what it does need: it needs, when it goes over to storage to
...
storage, which are a binary format that has been compressed and very
efficient.  Thousands and thousands of datoms per segment.  And we
keep that in an on- or off-heap cache.
...
happening is that you get something that looks like a map.  You can
ask for its keys.  You can do "get" and keyword lookup on it.  But it
is effectively a multi-map.  Because when you look at Datomic, because
we have multi-valued attributes, you can say I like ice cream, I like
pizza, I like whatever.  The "like" attribute is multi-valued.  The
maps that result from this are also what are called multi-maps: one
key can map to more than one value.  When it is a multi-valued key,
the value you get back you can consider like a set of values.
...
One of the neat things we have, though, is reverse attributes.  So we
talked about there being this inverse index, that we can point
backwards.  And that you cannot do with Clojure maps, obviously,
because the map knows what its things are, but because there is
actually a set underneath it, we can go backwards.
...
Datomic would be suitable.  If you need arbitrary write scaling, it is
not the right system.  But people that are choosing this, are choosing
it because they want ACID, and they want transactions, and they want
queries, and all of the other things that they would otherwise have to
give up if they did that.

And you would make that highly available in the traditional manner
with a standby machine, which we support.
...
peers.  You get more query.  And you can tbd scale queries depending
on the storage you choose.  A storage like DynamoDB really -- it has a
knob.  Unfortunately when you turn it, it costs more money, but it is
still really cool to have a knob.  And that is what you want.
...
The testing story is really interesting.  Probably a whole separate
...
talking about the value of tests in terms of information theory.  So
if you write a test, you know it will always work.  How much
...
It appears that "the prior talk" mentioned above is:
"Embracing Variability" by Don Reinertsen
https://www.infoq.com/presentations/Embracing-Variability ]

That is a really really important point.  It does not take away from the value of that test as a regression barricade.  But generative testing is really good for figuring out if you got it right in the first place, because it is generative.  And _you_ missed the talk, and it was awesome.

[Rich was pointing at someone in the audience at this point, probably
Stuart Halloway, because it sounds like his voice responding.]

[Audience member: I was comfortably asleep, I am sure.]

It was great.  Really great.

So we do that, and we do a lot of functional testing.  We do not do a
lot of the unit testing where you say: it should obviously do this.  I
hope it does it forever.
...
The first thing that I would like to just talk about is the fact that
being a simple system, and using Clojure the way we have, was a
definite source of agility.  I do not think these two things get
connected enough, but it was another critical thing that was in the
last talk.  He talked about margin.  And if you increase your
capacity, you can deal with more variability.  But then he said: don't
stop there.  And the very next slide, the very next slide was an
argument for simplicity in software.  It was a big involved slide, but
the point of it was: your degree of architectural independence is
going to dramatically improve your ability to deal with variability,
which is what we consider agility to be.  Can you do something when
things change?  Without redoing them, without rework?  If you can ever
get his slide deck, the slide after the leverage slide, where he
talked about capacity, he said the other thing to mitigate variability
in your process is: isolation of components, architectural isolation
of components.
...
In two weeks.  A huge architectural change to the system.  We were
running our own clusters, and everything.  Just like that it vanished.
...
Leveraging Clojure.  All the traditional leverage points that you see
in Lisps, and now more specifically Clojure.

Did we use print / read on Clojure data?  Relentlessly, because it is
a cheap way to get serialization?  Absolutely.  It is awesome.  It is
...
in a whole bunch of things, but when you get more specific, you should
always consider that.  Oh, I am doing a vector-like thing.  Should you
support nth?  Should you support Indexed?  You should.  If you do, you
are going to get some benefits out of doing that.

Obviously we used interop extensively, and that paid off for us.

And you can see how we have extended the state model.
...
So in summary, Clojure was made for this kind of app.  It is not
surprising, but Clojure was not made with this app in mind.  But this
category of application.  It needs to be very fast.  It is a large
system, but no part of it is large.

There is a ton of concurrency.  If it was not evident.  A ton of
concurrency.  We never ever -- right, Stu? -- we never sweated about
concurrency.  Never.  It is never one of our problems, because
everything is immutable, and when we need coordination, we use one of
the constructs like atoms to make that straightforward.
...
server, we now have to pass on to our customers, who consume the peer
library.  Of course most of their consuming applications are
themselves servers, but it is still something I would like to improve.
...
resulting application was simple.  And I think the same benefits will
be available to any application of a similar size written in this way.
...
new back ends, or would you guys do that?]
...
I refine how that looks, so publishing it would sort of be pouring
concrete on it.  It would be premature.  But certainly anybody who
wants a back end supported should talk to us.  We are interested in
doing it, and obviously we have already spoken about CouchBase.  I
think it is logical.

[Audience member: You mentioned that the protocols are all small.
Could you give us an example of one of the protocols?]
...
to narrower and narrower protocols?]

I did the cluster protocol first.  So I worked the state model very
...
implement it that way.  But I tried to make that work, so that is how
I ended up with that seven entry point thing.

And then we implemented a Dynamo storage cluster, and it was our only
intended -- so it was an implementation of that protocol.  And even
though we only had one, there was a protocol for it.
...
point in time I was doing Dynamo, I needed less of it than this first
protocol.  So it was a refactoring job to create that storage
protocol.  It did not change this one, but it put another layer in.
And then that one is the one that is really trivial to make
...
[Audience member: Stuart Halloway - TBD cluster.  When we only had the
distributed storage system, I was TBD about testing.  TBD or something
like that.  His hair stood on end.  And he came back the next day.
This is a great example of Clojure protocols.  He had taken the
cluster protocol and extended it back to concurrent hash map in Java,
which gave you a conformant implementation of the entire stack that
ran in memory, which is how I do the small localized testing.  Because
you do not ever have to mock or stub for performance reasons, because
you can TBD and it is just backed by collections.  And that was a
trivial job to do with Clojure protocols.  It would have been
potentially quite tricky with a different implementation.]

Right.  So there are protocols or interfaces around datoms, a peer --
what appears to be a peer, so we can swap peers, even if they do not
have any of the same infrastructure behind them at all.  They satisfy
the same communications.

I think it is critically important.  You should always put a protocol
or interface between any two things in your system, if nothing else.

And if you do data driven programming, you can also then put a queue
in, or a wire in between two things.

And those two guidelines will solve 90% of your architectural
problems.

[Audience member: Are there examples of TBD or TBD?]

We just saw the benchmarking thing yesterday with the Yahoo thing,
whatever, and we intend to implement that so we can show some
comparable things.  But we have not done Java pet store or anything,
no.

[Stuart Halloway: There is a small template on the web site that uses
the Seattle data.]
...
[Audience member: Is it implemented or TBD]
...
assertions of transactions.  If you want to assert an attribute of a
transaction, which is its business time, you can do that.  But the
...
what you need to do, if the transaction is coordinated with that, you
make it an attribute of the transaction it is very efficient.  If you
added a thousand things in the transaction, you get from those facts
to the transaction, and then to the business time, very efficiently.
Or any other fact about the transaction.  Business time, business
user, process, business approval.  Put them on the transaction.

[Audience member: A transaction has natural limitations.  Let us say
you have 20 cores.  Do you have numbers to show us that this is not
the bottleneck?]
...
[Audience member: Why is it not a problem?]
...
are like 99% of businesses that could not saturate one box with the
novelty that you are putting in the system, this was a good fit.  It
is that simple.
